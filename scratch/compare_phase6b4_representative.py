...
def run_comparison():
    ...
    for name, req in CASES:
        ...
        results[name] = {
            ...
            "aspect_ratio": getattr(spec.wing, "aspect_ratio", None),
            "root_chord_m": getattr(spec.wing, "root_chord", None),
        ...
        print(f"[{name}] success={res.success} MTOW={results[name]['mtow_kg']}kg WingArea={results[name]['wing_area_m2']}m2 Batt={results[name]['battery_name']} Endur={results[name]['endurance_min']}min iter={res.iterations}")

    with open("scratch/phase6b4_representative_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("Saved to scratch/phase6b4_representative_results.json")

    # Compare with Phase 6B-3 baseline
    with open("scratch/phase6b3_representative_results.json", "r") as f:
        b3 = json.load(f)

    print("\n--- PHASE 6B-3 vs PHASE 6B-4 COMPARISON ---")
    all_match = True
    for name in results:
        b3_case = b3.get(name, {})
        b4_case = results[name]
        diffs = []
        for key in ["mtow_kg", "wing_area_m2", "wingspan_m", "battery_name", "endurance_min", "cruise_power_w", "iterations"]:
            val_b3 = b3_case.get(key)
            val_b4 = b4_case.get(key)
            if val_b3 != val_b4:
                diffs.append(f"{key}: {val_b3} -> {val_b4}")
        if diffs:
            print(f"[DIFF] {name}: {', '.join(diffs)}")
            all_match = False
        else:
            print(f"[MATCH] {name}: MTOW={b4_case['mtow_kg']}kg, WingArea={b4_case['wing_area_m2']}m2, Endur={b4_case['endurance_min']}min - EXACT MATCH")

    if all_match:
        print("\nSUCCESS: All 7 representative cases match Phase 6B-3 baseline with 100% exact numerical equivalence!")
    else:
        print("\nWARNING: Some representative cases differ from Phase 6B-3 baseline.")
