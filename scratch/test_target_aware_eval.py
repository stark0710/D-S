...
# Let's see what happens if score_endur is target-aware in PropulsionObjectiveFunction
...
def target_aware_evaluate(self, candidate, context):
    ...
    # Requirement is a hard requirement: if below target, severe penalty
    if flight_time < t_target:
        score_endur = max(0.0, (flight_time / t_target) * 0.4)
    else:
        # Ideal operating reserve margin is 10% to 30% above target
        # Beyond 30% above target, score plateaus at 1.0 so weight penalty controls
        excess_ratio = (flight_time - t_target) / t_target
        if excess_ratio <= 0.30:
            score_endur = 0.85 + (excess_ratio / 0.30) * 0.15
        else:
            score_endur = 1.0  # Full credit for meeting target + reserve; further excess gains no score
...
for t in times:
    ...
    print(f"Target: {t:4.1f} min | MTOW: {mtow:5.3f} kg | Batt: {batt_name} ({batt_cap} mAh, {batt_w} g) | Achieved: {endurance:5.1f} min")
