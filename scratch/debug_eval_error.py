...
def debug_check(candidate, context):
    err = candidate.derived_variables.get("evaluation_error")
    print(f"Cand {candidate.design_variables['battery']['name']}: status={candidate.status}, err={err}")
    return orig_check(candidate, context)
