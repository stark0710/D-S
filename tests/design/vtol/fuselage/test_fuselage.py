...
@pytest.fixture
def valid_fuselage_requirements(dummy_mission_result, dummy_configuration_result, dummy_wing_result, dummy_airfoil_result, dummy_tail_result):
    return FuselageRequirements(
...
def test_fuselage_strategy_selection():
    """Verify registry returns correct strategy for cargo fuselage designs."""
    strategy = VTOLFuselageStrategyRegistry.get(VTOLMissionCategory.CARGO)
    assert strategy.category == VTOLMissionCategory.CARGO
    assert strategy.default_fuselage_type == "Box Fuselage"
    assert strategy.default_shape == "Rectangular"
    assert strategy.get_compartment_shares()["payload"] == 0.55


def test_fuselage_engine_flow(valid_fuselage_requirements):
    """Verify selection, compartments, layouts, mounting interfaces, and CG center offset."""
    engine = FuselageEngine()
    result = engine.design_fuselage(valid_fuselage_requirements)

    assert result is not None
    # check sized dimensions
    assert result.fuselage_geometry.fuselage_type == "Box Fuselage"  # CARGO category default
    assert result.fuselage_geometry.length_m > 0.0
    assert result.fuselage_geometry.volume_m3 > 0.0

    # check compartments
    assert result.compartment_layout.battery_bay.volume_m3 > 0.0
    assert result.compartment_layout.payload_bay.volume_m3 > 0.0

    # check internal placements and calculated CG center
    assert len(result.internal_layout.placements) == 4
    assert result.internal_layout.center_of_gravity_x_m > 0.0

    # check hardpoint mounting interfaces
    assert result.mounting_interfaces.wing_attachment is not None
    assert result.mounting_interfaces.tail_attachment is not None

    # check cooling inlets
    assert len(result.cooling_layout.inlets) >= 2
    assert result.cooling_layout.inlets[0].area_cm2 > 0.0

    # check static CG offset analysis
    assert abs(result.engineering_analysis.cg_offset_from_wing_mac_percent) <= 10.0


def test_fuselage_validator_failures(valid_fuselage_requirements):
    """Verify validator flags excessive CG travel or packing density."""
    engine = FuselageEngine()
    reqs = valid_fuselage_requirements

    # Alter requirements to enforce a negative maximum CG offset limit to force validation trigger
    reqs.metadata["max_cg_offset"] = -1.0

    with pytest.raises(FuselageValidationError) as excinfo:
        engine.design_fuselage(reqs)
    assert "exceeds maximum allowable static margin travel" in str(excinfo.value)
