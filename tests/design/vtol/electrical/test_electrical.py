...
from backend.design.vtol.electrical import (
...
@pytest.fixture
def dummy_forward_result():
    analysis = ForwardPropulsionAnalysis(required_thrust_n=15.0, available_thrust_n=45.0, static_thrust_n=60.0, noise_level_db=72.0, thermal_temperature_c=65.0)
    power = CruisePowerAnalysis(required_cruise_power_w=600.0, current_draw_a=13.5, power_loading_w_kg=20.0, battery_c_rate=2.5)
    perf = CruisePerformanceAnalysis(max_speed_kmh=120.0, rate_of_climb_m_s=3.5, climb_angle_deg=12.0, propulsive_efficiency=0.70, cruise_range_margin_km=15.0)
    layout = ForwardPropulsionLayout(placements=[], architecture="Single Pusher")
    return ForwardPropulsionResult(
        motor_selection={},
        propeller_selection={},
        esc_selection={},
...
@pytest.fixture
def valid_electrical_requirements(dummy_mission_result, dummy_configuration_result, dummy_wing_result, dummy_airfoil_result, dummy_tail_result, dummy_fuselage_result, dummy_lift_system_result, dummy_forward_result):
    return ElectricalRequirements(
        ...
        forward_propulsion_result=dummy_forward_result,
    )


def test_electrical_strategy_selection():
    """Verify registry returns correct strategy for Cargo electrical designs."""
    strategy = VTOLElectricalStrategyRegistry.get(VTOLMissionCategory.CARGO)
    assert strategy.category == VTOLMissionCategory.CARGO
    assert strategy.default_chemistry == "LiPo"
    assert strategy.default_reserve_factor == 1.25


def test_electrical_engine_flow(valid_electrical_requirements):
    """Verify battery sizing, power budgets, voltage sag, protection, and charging times."""
    engine = ElectricalEngine()
    result = engine.design_electrical_system(valid_electrical_requirements)

    assert result is not None
    # check battery selection configuration
    assert result.battery_pack.chemistry == "LiPo"  # CARGO requires LiPo
    assert result.battery_pack.series_count_s == 12
    assert result.battery_pack.parallel_count_p > 0
    assert result.battery_pack.energy_wh >= 1800.0 * 1.25

    # check power budget sums
    assert result.power_budget.total_hover_w > 0.0
    assert result.power_budget.total_cruise_w > 0.0
    assert result.power_budget.reserve_energy_wh > 0.0

    # check distribution rails
    assert len(result.power_distribution.buses) == 2
    assert result.power_distribution.high_current_routing_awg in (8, 10, 12)

    # check protections
    assert len(result.metadata["protection"].fuses) >= 2

    # check thermals & balancing charging
    assert result.thermal_analysis.hover_heat_generation_w > 0.0
    assert result.charging_analysis.charging_time_hr > 0.0


def test_electrical_validator_failures(valid_electrical_requirements):
    """Verify validator flags insufficient reserve capacity."""
    engine = ElectricalEngine()
    reqs = valid_electrical_requirements

    # Alter requirements to enforce a very high reserve energy fraction limit of 9.0 (900%)
    reqs.metadata["min_reserve_energy_fraction"] = 9.0

    with pytest.raises(ElectricalValidationError) as excinfo:
        engine.design_electrical_system(reqs)
    assert "Battery capacity failure" in str(excinfo.value)
