...
from backend.design.fixed_wing.report.report_strategy import EngineeringReviewStrategy
...
from backend.design.fixed_wing.report.report_result import ReportResult as ReportEngineResult
...
def dummy_engineering_results():
    ...
    cad_res = CADResult("shape", "assembly", {"wing": "s", "fuselage": "s", "tail": "s"}, "tree", "ref", {}, {}, cad_meta)

    mfg_res = ManufacturingResult(
        bill_of_materials=[{"part_number": "TW-FW-001", "name": "Wing", "quantity": 1}],
        manufacturing_drawings={"wing": "/tmp/wing.pdf"},
        assembly_documents=["guide.md"],
        exploded_views=["exploded.obj"],
        fabrication_files={"3D Printing": "/tmp/mount.stl"},
        manufacturing_analysis={"manufacturability_rating": "Moderate"},
        manufacturing_cost=850.0,
        quality_checklist=["CG check", "Thrust check"],
        procurement_list=[{"part_number": "TW-FW-COM-101", "name": "Motor", "quantity": 1}],
    )

    return (m_result, c_result, w_result, a_result, t_result, fuse_res,
            p_result, av_result, pay_result, mass_result, flight_res,
            ver_res, cad_res, mfg_res)


def test_report_strategy_selection():
    """Verify strategy registry fetches correct report strategies."""
    strategy = ReportStrategyRegistry.get("engineering review")
    assert isinstance(strategy, EngineeringReviewStrategy)
    assert strategy.name == "Engineering Review Strategy"
    assert "Appendix" in strategy.get_sections_list()


def test_report_validation_rules():
    """Test validator bounds for empty summaries and missing chapters."""
    validator = ReportValidator()

    # Case 1: Empty executive summary fails validation
    with pytest.raises(ReportValidationError) as excinfo:
        validator.validate(
            exec_summary="",
            sections={"Executive Summary": "x", "Mission Overview": "x",
                       "Configuration": "x", "Wing Geometry": "x", "a": "x"},
            min_sections=5,
            exported_files={},
        )
    assert "Executive Summary is empty" in str(excinfo.value)

    # Case 2: Insufficient chapter count
    with pytest.raises(ReportValidationError) as excinfo:
        validator.validate(
            exec_summary="Summary",
            sections={"Executive Summary": "x", "Mission Overview": "x",
                       "Configuration": "x", "Wing Geometry": "x"},
            min_sections=10,
            exported_files={},
        )
    assert "below the minimum" in str(excinfo.value)


def test_section_compilers(dummy_engineering_results):
    """Test that individual section compilers produce non-empty markdown content."""
    (m, c, w, a, t, f, p, av, pay, mass, flight, ver, cad, mfg) = dummy_engineering_results
    reqs = ReportRequirements(
        mission_result=m, configuration_result=c, wing_result=w,
        airfoil_result=a, tail_result=t, fuselage_result=f,
        propulsion_result=p, avionics_result=av, payload_result=pay,
        mass_result=mass, flight_result=flight, verification_result=ver,
        cad_result=cad, manufacturing_result=mfg,
    )

    exec_text = ExecutiveSummaryCompiler().compile(reqs)
    assert "Executive Summary" in exec_text
    assert "Wingspan" in exec_text

    mission_text = MissionSectionCompiler().compile(reqs)
    assert "Mission Overview" in mission_text
    assert "SURVEY" in mission_text


def test_report_engine_flow(dummy_engineering_results):
    """Test full integration loop in ReportEngine."""
    (m, c, w, a, t, f, p, av, pay, mass, flight, ver, cad, mfg) = dummy_engineering_results
    reqs = ReportRequirements(
        mission_result=m, configuration_result=c, wing_result=w,
        airfoil_result=a, tail_result=t, fuselage_result=f,
    ...
    engine = ReportEngine()
    result = engine.process_engineering_report(reqs)

    assert isinstance(result, ReportEngineResult)

    # Verify sections compiled
    assert len(result.report_sections) >= 10
    assert "Executive Summary" in result.report_sections
    assert "Mission Overview" in result.report_sections
    assert "Wing Geometry" in result.report_sections
    assert "Flight Performance" in result.report_sections
    assert "Manufacturing Package" in result.report_sections

    # Verify exports produced
    assert "PDF" in result.exported_files
    assert "HTML" in result.exported_files
    ...
    for fmt, path in result.exported_files.items():
        ...
        assert os.path.getsize(path) > 0, f"{fmt} file is empty at {path}"

    # Verify engineering statistics
    assert result.engineering_statistics["wingspan_m"] == 2.0
    assert result.engineering_statistics["aspect_ratio"] == 10.0
    assert result.engineering_statistics["compliance_rating_pct"] == 90.0

    # Verify executive summary content is populated
