...
from backend.design.fixed_wing.cad.cad_strategy import ManufacturingCADStrategy
...
from backend.design.fixed_wing.cad.cad_result import CADResult as CADEngineResult
...
def dummy_engineering_results():
    ...
    return m_result, c_result, w_result, a_result, t_result, fuse_res, p_result, av_result, pay_result, mass_result, flight_res, ver_res


def test_cad_strategy_selection():
    """Verify strategy registry fetches correct CAD strategies."""
    strategy = CADStrategyRegistry.get("manufacturing")
    assert isinstance(strategy, ManufacturingCADStrategy)
    assert strategy.name == "Manufacturing"


def test_cad_validation_rules():
    """Test validator bounds for empty volumes and missing parts."""
    validator = CADValidator()

    # Case 1: Empty volume fails CAD validation
    with pytest.raises(CADValidationError) as excinfo:
        validator.validate(0.0, (1.5, 2.0, 0.4), {"wing": "shape", "fuselage": "shape", "tail": "shape"}, {}, ["STEP"])
    assert "calculated volume" in str(excinfo.value)

    # Case 2: Missing tail fails validation
    with pytest.raises(CADValidationError) as excinfo:
        validator.validate(0.15, (1.5, 2.0, 0.4), {"wing": "shape", "fuselage": "shape"}, {}, ["STEP"])
    assert "Missing core assembly" in str(excinfo.value)


def test_cad_engine_flow(dummy_engineering_results):
    """Test full integration loop in CADEngine."""
    m, c, w, a, t, f, p, av, pay, mass, flight, ver = dummy_engineering_results
    reqs = CADRequirements(
        mission_result=m,
    ...
    engine = CADEngine()
    result = engine.process_cad_generation(reqs)

    assert isinstance(result, CADEngineResult)
    assert result.cad_metadata.volume_m3 > 0.0
    assert len(result.parts) >= 3
    assert "wing" in result.parts
    assert "fuselage" in result.parts
    assert "tail" in result.parts
    assert len(result.exported_files) > 0
    
    # Assert physical file existence on disk
